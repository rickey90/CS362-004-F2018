...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case steward:
		return callSteward(state, choice1, choice2, choice3, handPos);
		/*
      if (choice1 == 1)
	{
	  //+2 cards
    ...
    case embargo: 
		return callEmbargo(state, choice1);
	/*
      //+2 Coins
      state->coins = state->coins + 2;
...
int callGardens(struct gameState *state)
{
	return -1;
}
	 
int callEmbargo(struct gameState *state, int choice1)
{
	//+2 Coins
	state->coins = state->coins + 2;

	//see if selected pile is in play
	if (state->discardCount[choice1] == -1)
	{
		return -1;
	}
	return 0; 
...
int callSteward(struct gameState *state, int choice1, int choice2, int choice3, int handPos)
{
	int currentPlayer = whoseTurn(state);
